  ...
  describe("generateDevelopmentReport", () => {
    it("should generate a valid base64 PDF with default example data", async () => {
      ...
      assert.ok(
        ...
        "Result should be a valid base64-encoded PDF",
      );
    });
    ...
    it("should generate a valid PDF for female participant", async () => {
      ...
      assert.ok(
        isValidBase64PDF(result),
        "Should generate valid PDF for female participant",
      );
    });
    ...
    it("should handle different recruitment profiles", async () => {
      ...
      for (const profile of profiles) {
        try {
          const participant = createTestParticipant({
            recruitmentProfile: profile as any,
          });
          const result = await generateDevelopmentReport(participant);

          assert.ok(
            isValidBase64PDF(result),
            `Should generate valid PDF for ${profile} profile`,
          );
        } catch (error) {
    ...
    it("should handle various HEXACO score ranges", async () => {
      const testCases = [
        {
          name: "Low scores",
          scores: { H: 1.5, E: 1.8, X: 1.2, A: 1.9, C: 1.7, O: 1.4 },
        },
        {
          name: "Medium scores",
          scores: { H: 3.0, E: 3.0, X: 3.0, A: 3.0, C: 3.0, O: 3.0 },
        },
        {
          name: "High scores",
          scores: { H: 4.8, E: 4.9, X: 4.7, A: 4.6, C: 4.8, O: 4.9 },
        },
      ];
    ...
    it("should include optional discrepancyHexaco when provided", async () => {
      ...
      assert.ok(
        isValidBase64PDF(result),
        "Should handle optional discrepancyHexaco field",
      );
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle missing ideal candidate results gracefully", async () => {
      ...
      try {
        ...
        assert.ok(
          isValidBase64PDF(result),
          "Should handle empty idealCandidateResults",
        );
      } catch (error) {
