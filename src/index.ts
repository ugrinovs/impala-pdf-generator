...
const DOCX_TEMPLATE = path.join(
  BASE_DIR,
  "Rastimir_Potencijalovic_Recruitment_Development_Flow_Executive_Summary.docx",
);
const IMPALA_OUTPUT = path.join(BASE_DIR, "Impala_OUTPUT.xlsx");
const IDEAL_CANDIDATE = path.join(
  BASE_DIR,
  "Idealan kandidat atributi (1).xlsx",
);
 ...
 */
async function extractDataFromImpalaOutput(): Promise<{
  candidate_name: string;
  profile_type: string;
  gender_pronoun: string;
}> {
 ...
 */
async function calculateScoresFromIdealCandidate(
  ...
  if (scores.hexaco_dimensions.length > 0) {
    const totalFitIndex = scores.hexaco_dimensions.reduce(
      (sum, dim) => sum + dim.fit_index,
      0,
    );
    scores.fit_index =
      Math.round((totalFitIndex / scores.hexaco_dimensions.length) * 1000) / 10; // Convert to percentage
  }
...
async function fillDocxTemplate(
  ...
  if (
    ...
    documentXml = documentXml.replace(
      /\bHis\b/g,
      genderPronoun === "Hers" ? "Her" : genderPronoun,
    );
  }
  ...
  const traitMapping = [
    {
      ...
      trait: "Integrity and trust",
      discrepancy: "Low",
    },
    {
      dimension: "Emotionality",
      trait: "Emotional regulation and resilience",
      discrepancy: "Moderate",
    },
    {
      dimension: "Extraversion",
      trait: "Communication and influence",
      discrepancy: "Low",
    },
    {
      dimension: "Agreeableness",
      trait: "Cooperation and diplomacy",
      discrepancy: "Moderate",
    },
    {
      dimension: "Conscientiousness",
      trait: "Performance and reliability",
      discrepancy: "Moderate",
    },
    {
      dimension: "Openness to Experience",
      trait: "Learning and innovation",
      discrepancy: "High",
    },
  ];
  ...
  if (valueReplacementCount !== expectedReplacements) {
    console.warn(
      `Warning: Expected ${expectedReplacements} value replacements but made ${valueReplacementCount}`,
    );
  }
...
function convertDocxToPdf(docxPath: string): string | null {
  ...
  try {
    execSync(
      `libreoffice --headless --convert-to pdf --outdir ${OUTPUT_DIR} ${docxPath}`,
      {
        stdio: "pipe",
      },
    );
...
export async function generatePDF(
  ...
  if (missingFiles.length > 0) {
    throw new Error(
      `Missing required files: ${missingFiles.map((f) => path.basename(f)).join(", ")}`,
    );
  }
  ...
  if (fleetDir) {
    if (
      !fs.existsSync(fleetDir) ||
      fs.readdirSync(fleetDir).filter((f) => f.endsWith(".pdf")).length === 0
    ) {
      console.warn("\nWarning: No PDF files found in Fleet-15 directory");
      console.warn("The final PDF will only contain the generated report.");
    } else if (
      fleetDir === FLEET_DIR &&
      candidateData.candidate_name.toLowerCase() !== FLEET_CANDIDATE_NAME
    ) {
      console.warn("\n" + "!".repeat(60));
      console.warn(
        "WARNING: Using default Fleet-15 PDFs with a different candidate!",
      );
      console.warn(
        `The Fleet-15 PDFs contain assessment data for "${FLEET_CANDIDATE_NAME.charAt(0).toUpperCase() + FLEET_CANDIDATE_NAME.slice(1)}",`,
      );
      console.warn(
        `but you're generating a report for "${candidateData.candidate_name}".`,
      );
      console.warn(
        `To exclude Fleet PDFs, pass null as the fleetPdfsPath parameter.`,
      );
  ...
  fs.writeFileSync(
    path.join(outputDir, "candidate_data.json"),
    JSON.stringify(candidateData, null, 2),
  );
  fs.writeFileSync(
    path.join(outputDir, "calculated_scores.json"),
    JSON.stringify(scores, null, 2),
  );
  fs.writeFileSync(
    path.join(outputDir, "filled_report.docx"),
    fs.readFileSync(filledDocx),
  );

  // Step 3: Convert DOCX to PDF
  const generatedPdf = convertDocxToPdf(filledDocx);

  console.log("filledDocx:", filledDocx);

  // Step 4: Merge with Fleet-15 PDFs (if fleetDir is not null)
  const finalPdf = await mergePdfsWithPaths(
    generatedPdf,
...
async function mergePdfsWithPaths(
  ...
  const fleetPdfs =
    fleetDir && fs.existsSync(fleetDir)
      ? fs
  ...
  const finalPdfPath = path.join(
    outputDir,
    `${candidateName}_Complete_Report.pdf`,
  );
